 ...
 */
import { listDueHooks, getPerson, markHookFired, markHookSkipped, appendOutbound, logOutreach } from './store.js';
...
async function processHook(hook) {
  ...
  if (!gate.pass) {
    ...
    markHookSkipped(hook.id, gate.reason);
    return;
  }
  ...
  if (!decision.send) {
    ...
    markHookSkipped(hook.id, decision.reason);
    return;
  }

  try {
    if (sendCard) await sendCard(openId, decision.message);
    ...
    // 发送失败不 markFired，下轮重试（但已 log skip 前的判断不重复）
  }
}
